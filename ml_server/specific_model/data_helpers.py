...
def get_hour(item):
    ...
    """
    try:
        return pd.to_datetime(item, errors="coerce").hour
    except Exception as e:
        logging.warning(f"Failed to parse hour from {item}: {e}")
        return np.nan


def date_to_ordinal(data):
    """
    Converstion of a datetime value into integer representation
...
def feature_engineering(data):
    ...
    if "date" in data.columns:
        ...
        data["date_ordinal"] = data["date"].apply(date_to_ordinal)
    ...
    if "quantity_wasted" in data.columns and "quantity_ordered" in data.columns:
        data["wastage_rate"] = data["quantity_wasted"] / data[
            "quantity_ordered"
        ].replace({0: np.nan})
